...
func TestDialSuccess(t *testing.T) {
	...
	client, err := Dial(WithServers(fmt.Sprintf("tcp://%s", addr)), WithClientID("test-client"))
	require.NoError(t, err)
...
func TestDialWithCredentials(t *testing.T) {
	...
	client, err := Dial(WithServers(fmt.Sprintf("tcp://%s", addr)),
...
func TestDialContext(t *testing.T) {
	...
	t.Run("success with context", func(t *testing.T) {
		...
		client, err := DialContext(ctx, WithServers(fmt.Sprintf("tcp://%s", addr)), WithClientID("ctx-client"))
		require.NoError(t, err)
		require.NotNil(t, client)
		defer client.Close()
...
func TestClientParentContextPropagation(t *testing.T) {
	...
	t.Run("client closes when parent context canceled", func(t *testing.T) {
		...
		client, err := DialContext(parentCtx, WithServers(fmt.Sprintf("tcp://%s", addr)), WithClientID("test-client"))
		require.NoError(t, err)
...
func TestClientCancelOnDialErrors(t *testing.T) {
	...
	t.Run("cancel called on CONNACK failure", func(t *testing.T) {
		...
		_, err := Dial(WithServers(fmt.Sprintf("tcp://%s", addr)), WithClientID("test-client"))
		assert.Error(t, err)
	...
	t.Run("cancel called on invalid CONNACK", func(t *testing.T) {
		...
		_, err := Dial(WithServers(fmt.Sprintf("tcp://%s", addr)),
			WithClientID("test-client"),
			WithConnectTimeout(100*time.Millisecond),
		)
		assert.Error(t, err)
...
func TestClientQoS2PubrecErrorHandling(t *testing.T) {
	...
	t.Run("sends PUBREL even when PUBREC has error reason code", func(t *testing.T) {
		...
		client, err := Dial(WithServers(fmt.Sprintf("tcp://%s", addr)), WithClientID("test"))
		require.NoError(t, err)
		defer client.Close()
...
func TestDialValidation(t *testing.T) {
	...
	t.Run("with server resolver succeeds validation", func(t *testing.T) {
		...
		resolver := func(_ context.Context) ([]string, error) {
			return []string{fmt.Sprintf("tcp://%s", addr)}, nil
		}
...
func TestClientReconnectLoop(t *testing.T) {
	...
	t.Run("reconnects successfully on first attempt", func(t *testing.T) {
		...
		client, err := Dial(
			WithServers(fmt.Sprintf("tcp://%s", addr)),
	...
	t.Run("resends inflight messages when session present", func(t *testing.T) {
		...
		client, err := Dial(
			WithServers(fmt.Sprintf("tcp://%s", listener.Addr().String())),
			WithAutoReconnect(true),
			WithReconnectBackoff(10*time.Millisecond),
			WithConnectTimeout(100*time.Millisecond),
		)
...
func TestClientDialDefaultPorts(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(_ *testing.T) {
			...
			_, _ = DialContext(ctx, WithServers(fmt.Sprintf("%s://nonexistent", tt.scheme)))
...
func TestClientDialWithProxy(t *testing.T) {
	...
	t.Run("TCP with proxy", func(t *testing.T) {
		...
		client, err := DialContext(ctx,
			WithServers(fmt.Sprintf("tcp://%s", targetAddr)),
			WithProxy(proxyURL),
		)
		require.NoError(t, err)
		defer client.Close()
	...
	t.Run("TLS with proxy", func(t *testing.T) {
		...
		client, err := DialContext(ctx,
			WithServers(fmt.Sprintf("tls://%s", targetAddr)),
			WithProxy(proxyURL),
			WithTLS(clientTLS),
		)
		require.NoError(t, err)
...
func TestClientDialUnixSocket(t *testing.T) {
	...
	testUnixSocketDial := func(t *testing.T, prefix string) {
		...
		socketPath := fmt.Sprintf("/tmp/mqtt_test_%s_%d.sock", prefix, time.Now().UnixNano())
		...
		client, err := DialContext(ctx, WithServers(fmt.Sprintf("unix://%s", socketPath)))
		require.NoError(t, err)
		defer client.Close()
